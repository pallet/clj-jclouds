...
function log-without () {
    ...
    grep -v -f <(git --no-pager log --format=%H --grep=Merge --grep=travis --grep='Updated version for next' --grep='Updated project.clj, release notes and readme for' --grep='release script' $previous_version.. ) |
