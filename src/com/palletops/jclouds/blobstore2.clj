...
(ns com.palletops.jclouds.blobstore2
...
Here's a quick example of how to view resources in rackspace
    ...
    (use 'com.palletops.jclouds.blobstore2)
...
See http://code.google.com/p/jclouds for details."
  (:use [com.palletops.jclouds.core])
...
;; Something in clojure 1.3 (namespaces?) does not like a private type
;; called byte-array-type, so we refer to (class (make-array ...))
;; directly; and it only parses if it is its own block, hence
;; separating it from the above
(extend-protocol PayloadSource
     ...
     (let [blob-builder (.name (BlobBuilderImpl.) name)
           blob-builder (if payload
                          (.payload blob-builder
                                    (com.palletops.jclouds.blobstore2/payload
                                     payload))
